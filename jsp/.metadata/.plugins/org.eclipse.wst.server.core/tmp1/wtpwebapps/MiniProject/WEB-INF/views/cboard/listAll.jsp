...
<script>
	$(function(){
		...
		$(".pagingSize").change(function(){
			...
			location.href = '/cboard/listAll?pagingSize=' + $(this).val() + "&pageNo=" + pageNo;
		})
...
<%-- 	onchange="choosePagingSizeOption(${pagingInfo.pageNo}) --%>
	...
	<table class="table table-hover">
		...
		<div><button type="button" class="btn btn-success" onclick="location.href='/cboard/showSaveBoardForm';">글쓰기</button></div>
...
<%-- 		<div>${search }</div> --%>
		<div class ="paging">
			<ul class="pagination justify-content-center">
				...
				<c:if test = '${pagingInfo.pageNo > 1 }'>
			    <li class="page-item"><a class="page-link" href="/cboard/listAll?pageNo=${pagingInfo.pageNo - 1}&pagingSize=${param.pagingSize}&searchType=${search.searchType}&searchWord=${search.searchWord}">Previous</a></li>
			    </c:if>
			    <c:forEach var = "i" begin ='${pagingInfo.startPageNoCurBlock}' end='${pagingInfo.endPageNoCurBlock }'>
				    <li class="page-item"><a class="page-link" href="/cboard/listAll?pageNo=${i}&pagingSize=${param.pagingSize}">${i}</a></li>
			    </c:forEach>
			    <c:if test="${pagingInfo.pageNo < pagingInfo.totalPageCnt}">
			    <li class="page-item"><a class="page-link" href="/cboard/listAll?pageNo=${pagingInfo.pageNo + 1}&pagingSize=${param.pagingSize}&searchType=${search.searchType}&searchWord=${search.searchWord}">Next</a></li>
			    </c:if>
			    <c:if test="${pagingInfo.pageNo== pagingInfo.totalPageCnt}">
